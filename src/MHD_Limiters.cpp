...
namespace MHD_Limiters
{
					   ...
					   const State &a_W_ave)
	{
		for (int i = 0; i < NUMCOMPS; i++)
		{
			if (a_W_ave_high_limited(i) != a_W_ave(i))
				cout << a_pt[0] << a_pt[1] << endl;
		}
	}
		...
		int a_d)
	{
		...
		for (int i = 0; i < NUMCOMPS; i++)
		{
			...
			if (i == 4)
			{
				if (W_low_ahead_limited(i) < 0)
					W_low_ahead_limited(i) = 0.; // WP;
				if (W_high_limited(i) < 0)
					W_high_limited(i) = 0.; // WP;
			}
		}
	}
