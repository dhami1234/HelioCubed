...
namespace MHD_Riemann_Solvers
{
						...
						double a_gamma)
	{
		...
		if (iAveraging == 1)
		{
			...
			X = d_1_8PI * (DBX * DBX + DBY * DBY + DBZ * DBZ) * coefRL * coefRL / RhoL;

			cc = hgamma * (HAV - kk - d_1_4PI * (BX * BXAV + BYZ2) / Rho) + (two - gamma) * X;
		}
		else
		{
			Rho = half * (RhoL + RhoR);
		...
		du5 = -hgamma * (-kk * du1 + U * du2 + V * du3 + W * du4 - du5 + (BX * du6 + BY * du7 + BZ * du8) * d_1_4PI);
		...
		lu1 = Aeigen1 * (so11 * du1 + so12 * du2 + so13 * du3 + so14 * du4 + so15 * du5 + so16 * du6 + so17 * du7 + so18 * du8);
		lu2 = Aeigen2 * (so21 * du1 + so22 * du2 + so23 * du3 + so24 * du4 + so25 * du5 + so26 * du6 + so27 * du7 + so28 * du8);
		lu3 = Aeigen3 * (so31 * du1 + so32 * du2 + so33 * du3 + so34 * du4 + so35 * du5 + so36 * du6 + so37 * du7 + so38 * du8);
		lu4 = Aeigen4 * (so41 * du1 + so42 * du2 + so43 * du3 + so44 * du4 + so45 * du5 + so46 * du6 + so47 * du7 + so48 * du8);
		lu5 = Aeigen5 * (so51 * du1 + so52 * du2 + so53 * du3 + so54 * du4 + so55 * du5 + so56 * du6 + so57 * du7 + so58 * du8);
		lu6 = Aeigen6 * (so61 * du1 + so62 * du2 + so63 * du3 + so64 * du4 + so65 * du5 + so66 * du6 + so67 * du7 + so68 * du8);
		lu7 = Aeigen7 * (so71 * du1 + so72 * du2 + so73 * du3 + so74 * du4 + so75 * du5 + so76 * du6 + so77 * du7 + so78 * du8);
		lu8 = Aeigen8 * (so81 * du1 + so82 * du2 + so83 * du3 + so84 * du4 + so85 * du5 + so86 * du6 + so87 * du7 + so88 * du8);

		ra1 = sp11 * lu1 + sp12 * lu2 + sp13 * lu3 + sp14 * lu4 + sp15 * lu5 + sp16 * lu6 + sp17 * lu7 + sp18 * lu8;
		ra2 = sp21 * lu1 + sp22 * lu2 + sp23 * lu3 + sp24 * lu4 + sp25 * lu5 + sp26 * lu6 + sp27 * lu7 + sp28 * lu8;
		ra3 = sp31 * lu1 + sp32 * lu2 + sp33 * lu3 + sp34 * lu4 + sp35 * lu5 + sp36 * lu6 + sp37 * lu7 + sp38 * lu8;
		ra4 = sp41 * lu1 + sp42 * lu2 + sp43 * lu3 + sp44 * lu4 + sp45 * lu5 + sp46 * lu6 + sp47 * lu7 + sp48 * lu8;
		ra5 = sp51 * lu1 + sp52 * lu2 + sp53 * lu3 + sp54 * lu4 + sp55 * lu5 + sp56 * lu6 + sp57 * lu7 + sp58 * lu8;
		ra6 = sp61 * lu1 + sp62 * lu2 + sp63 * lu3 + sp64 * lu4 + sp65 * lu5 + sp66 * lu6 + sp67 * lu7 + sp68 * lu8;
		ra7 = sp71 * lu1 + sp72 * lu2 + sp73 * lu3 + sp74 * lu4 + sp75 * lu5 + sp76 * lu6 + sp77 * lu7 + sp78 * lu8;
		ra8 = sp81 * lu1 + sp82 * lu2 + sp83 * lu3 + sp84 * lu4 + sp85 * lu5 + sp86 * lu6 + sp87 * lu7 + sp88 * lu8;

		ra5 = ra1 * kk + Rho * (U * ra2 + V * ra3 + W * ra4) + ra5 / hgamma + d_1_4PI * (BX * ra6 + BY * ra7 + BZ * ra8);
		ra2 = U * ra1 + Rho * ra2;
		ra3 = V * ra1 + Rho * ra3;
		ra4 = W * ra1 + Rho * ra4;
		...
		int a_dir)
	{
		...
		a_flux = forall<double, NUMCOMPS, MEM, 1>([] PROTO_LAMBDA(
								  ...
								  const int a_order)
	{
		...
		if (a_order == 4)
		{
			...
			MHD_Riemann_Solvers::MHDSphericalFlux(F_f_low_mapped, a_W_ave_low, a_W_ave_low, a_W_ave_low_actual, a_W_ave_low_actual, DrDetAA4, DrDetAA4, a_Dr_detA_avg, a_Dr_detA_avg, a_Dr_adjA_avg, a_Dr_adjA_avg, a_gamma, a_d);
			MHD_Riemann_Solvers::MHDSphericalFlux(F_f_high_mapped, a_W_ave_high, a_W_ave_high, a_W_ave_high_actual, a_W_ave_high_actual, DrDetAA4, DrDetAA4, a_Dr_detA_avg, a_Dr_detA_avg, a_Dr_adjA_avg, a_Dr_adjA_avg, a_gamma, a_d);
		}
