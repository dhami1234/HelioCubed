...

    {
        ...
        for (auto dit : state.m_U)
        {
            ...
            if (inputs.Spherical_2nd_order == 0)
                dbx0 = dbx0.grow(-NGHOST);
            BoxData<double, NUMCOMPS> U_dim(dbx0);
            if (inputs.Spherical_2nd_order == 1)
                state.m_U[dit].copyTo(U_dim);
            if (inputs.Spherical_2nd_order == 0)
                MHD_Mapping::JU_to_U_ave_calc_func(U_dim, state.m_U[dit], state.m_r2rdot_avg[dit], state.m_detA_avg[dit]);
            ...
            if (dbx0.contains(index_cc))
            {
                for (int i = 0; i < NUMCOMPS; i++)
                {
                    ...
                    if (dbx0.contains(index_n0))
                        probed_values[i] += (U_dim(index_n0, i) - U_dim(index_cc, i)) * (r_now - x_sph_cc(index_cc, 0)) / (x_sph_cc(index_n0, 0) - x_sph_cc(index_cc, 0));
                    if (dbx0.contains(index_n1))
                        probed_values[i] += (U_dim(index_n1, i) - U_dim(index_cc, i)) * (latitude_now * c_PI / 180 - x_sph_cc(index_cc, 1)) / (x_sph_cc(index_n1, 1) - x_sph_cc(index_cc, 1));
                    if (dbx0.contains(index_n2))
                        probed_values[i] += (U_dim(index_n2, i) - U_dim(index_cc, i)) * (longitude_now * c_PI / 180 - x_sph_cc(index_cc, 2)) / (x_sph_cc(index_n2, 2) - x_sph_cc(index_cc, 2));
                }
