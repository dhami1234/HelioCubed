						...
						double &a_min_dt)
	{
		...
		for (auto dit : a_State.m_U)
		{
			...
			if (!a_State.m_divV_calculated && inputs.non_linear_visc_apply == 1)
				a_State.m_divV[dit].setVal(0.0);
			MHD_Mapping::JU_to_U_Sph_ave_calc_func(a_U_Sph_ave, a_JU_ave[dit], a_State.m_detAA_inv_avg[dit], a_State.m_r2rdot_avg[dit], a_State.m_detA_avg[dit], a_State.m_A_row_mag_avg[dit], false, 4);
			MHD_Mapping::JU_to_U_Sph_ave_calc_func(a_U_Sph_actual_ave, a_JU_ave[dit], a_State.m_detAA_inv_avg[dit], a_State.m_r2rdot_avg[dit], a_State.m_detA_avg[dit], a_State.m_A_row_mag_avg[dit], true, 4);
						   ...
						   double &a_min_dt)
	{
		...
		for (auto dit : a_State.m_U)
		{
			...
			for (int d = 0; d < DIM; d++)
			{
				...
				if (d == 0)
					MHD_Mapping::Spherical_to_Cartesian(F_f, F_f_sph, a_State.m_x_sph_fc_1[dit]);
				if (d == 1)
					MHD_Mapping::Spherical_to_Cartesian(F_f, F_f_sph, a_State.m_x_sph_fc_2[dit]);
				if (d == 2)
					MHD_Mapping::Spherical_to_Cartesian(F_f, F_f_sph, a_State.m_x_sph_fc_3[dit]);
			...
			if (!a_State.m_divB_calculated && inputs.takedivBstep == 1)
				forallInPlace_p(Powell_Sph_2O, a_State.m_divB[dit], W_cart, RhsV_divB, a_State.m_cell_volume[dit]);
			// Vector RHS(dbx1);
