	                         ...
	                         MHDLevelDataState& a_State)
	{
		...
		for (auto dit : a_State.m_U){	
			...
			if (dbx0.low()[0] < a_State.m_probDom.box().low()[0] && inputs.LowBoundType == 1) {
				if (inputs.sph_inner_BC_hdf5 == 1){
					...
					if (inputs.grid_type_global == 2 && inputs.initialize_in_spherical_coords == 1){
						MHD_Mapping::U_Sph_ave_to_JU_calc_func(a_JU_ghost, a_U_sph_scaled_r, a_State.m_detAA_avg[dit], a_State.m_A_avg[dit],a_State.m_detAA_inv_avg[dit], a_State.m_r2rdot_avg[dit], a_State.m_detA_avg[dit],a_State.m_A_row_mag_avg[dit], true, 2);
					} else {
					...
					if (inputs.grid_type_global == 2 && inputs.initialize_in_spherical_coords == 1){
						MHD_Mapping::U_Sph_ave_to_JU_calc_func(a_JU_ghost, a_U, a_State.m_detAA_avg[dit], a_State.m_A_avg[dit],a_State.m_detAA_inv_avg[dit], a_State.m_r2rdot_avg[dit], a_State.m_detA_avg[dit],a_State.m_A_row_mag_avg[dit], true, 2);
					} else {
			...
			if (dbx0.high()[0] > a_State.m_probDom.box().high()[0] && inputs.HighBoundType == 2) {
				...
				if (inputs.grid_type_global == 2 && inputs.initialize_in_spherical_coords == 1){
					MHD_Mapping::JU_to_U_Sph_ave_calc_func(a_U, a_JU[dit], a_State.m_detAA_inv_avg[dit], a_State.m_A_inv_avg[dit], a_State.m_r2rdot_avg[dit], a_State.m_detA_avg[dit],a_State.m_A_row_mag_avg[dit], false, 2);
				} else {
				...
				if (inputs.grid_type_global == 2 && inputs.initialize_in_spherical_coords == 1){
					MHD_Mapping::U_Sph_ave_to_JU_calc_func(a_JU_ghost, a_U_ghost, a_State.m_detAA_avg[dit], a_State.m_A_avg[dit], a_State.m_detAA_inv_avg[dit], a_State.m_r2rdot_avg[dit], a_State.m_detA_avg[dit], a_State.m_A_row_mag_avg[dit], false, 2);
				} else {
					a_JU_ghost = forall<double,NUMCOMPS>(dot_pro_calcF, a_State.m_Jacobian_ave[dit], a_U_ghost);
				}
				// a_JU_ghost = forall<double,NUMCOMPS>(dot_pro_calcF, a_State.m_Jacobian_ave[dit], a_U_ghost);
