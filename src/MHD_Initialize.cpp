...
namespace MHD_Initialize {
	...
	void initializeState_Spherical(MHDLevelDataState& a_State)
	{
		...
		for (auto dit : a_State.m_U){
			...
			// UBig +=  Lap;
			HDF5Handler h5;
			MHD_Mapping::U_Sph_ave_to_JU_calc_func(a_State.m_U[dit], UBig, a_State.m_detAA_avg[dit], a_State.m_A_avg[dit], a_State.m_detAA_inv_avg[dit], a_State.m_r2rdot_avg[dit], a_State.m_detA_avg[dit], a_State.m_A_row_mag_avg[dit], true, 2);
			MHDOp::DimToNonDimcalc(a_State.m_U[dit]);
		}
	}
