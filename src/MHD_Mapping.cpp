...
namespace MHD_Mapping
{
							 ...
							 const double a_dz)
	{
		...
		a_A_1_avg(0) = a_r2detAA_1_avg(0) / (a_r2detA_1_avg(0) + eps);
		a_A_1_avg(1) = a_r2detAA_1_avg(1) / (a_r2detA_1_avg(0) + eps);
		a_A_1_avg(2) = a_r2detAA_1_avg(2) / (a_r2detA_1_avg(0) + eps);
		a_A_1_avg(3) = a_r2detAA_1_avg(3) / (a_r2detA_1_avg(0) + eps);
		a_A_1_avg(4) = a_r2detAA_1_avg(4) / (a_r2detA_1_avg(0) + eps);
		a_A_1_avg(5) = a_r2detAA_1_avg(5) / (a_r2detA_1_avg(0) + eps);
		a_A_1_avg(6) = a_r2detAA_1_avg(6) / (a_r2detA_1_avg(0) + eps);
		a_A_1_avg(7) = a_r2detAA_1_avg(7) / (a_r2detA_1_avg(0) + eps);
		a_A_1_avg(8) = 0.0;
		...
		a_A_2_avg(0) = a_rrdotdetAA_2_avg(0) / (a_rrdotdetA_2_avg(0) + eps);
		a_A_2_avg(1) = a_rrdotdetAA_2_avg(1) / (a_rrdotdetA_2_avg(0) + eps);
		a_A_2_avg(2) = a_rrdotdetAA_2_avg(2) / (a_rrdotdetA_2_avg(0) + eps);
		a_A_2_avg(3) = a_rrdotdetAA_2_avg(3) / (a_rrdotdetA_2_avg(0) + eps);
		a_A_2_avg(4) = a_rrdotdetAA_2_avg(4) / (a_rrdotdetA_2_avg(0) + eps);
		a_A_2_avg(5) = a_rrdotdetAA_2_avg(5) / (a_rrdotdetA_2_avg(0) + eps);
		a_A_2_avg(6) = a_rrdotdetAA_2_avg(6) / (a_rrdotdetA_2_avg(0) + eps);
		a_A_2_avg(7) = a_rrdotdetAA_2_avg(7) / (a_rrdotdetA_2_avg(0) + eps);
		a_A_2_avg(8) = 0.0;
		...
		a_A_3_avg(0) = a_rrdotdetAA_3_avg(0) / (a_rrdotdetA_3_avg(0) + eps);
		a_A_3_avg(1) = a_rrdotdetAA_3_avg(1) / (a_rrdotdetA_3_avg(0) + eps);
		a_A_3_avg(2) = a_rrdotdetAA_3_avg(2) / (a_rrdotdetA_3_avg(0) + eps);
		a_A_3_avg(3) = a_rrdotdetAA_3_avg(3) / (a_rrdotdetA_3_avg(0) + eps);
		a_A_3_avg(4) = a_rrdotdetAA_3_avg(4) / (a_rrdotdetA_3_avg(0) + eps);
		a_A_3_avg(5) = a_rrdotdetAA_3_avg(5) / (a_rrdotdetA_3_avg(0) + eps);
		a_A_3_avg(6) = a_rrdotdetAA_3_avg(6) / (a_rrdotdetA_3_avg(0) + eps);
		a_A_3_avg(7) = a_rrdotdetAA_3_avg(7) / (a_rrdotdetA_3_avg(0) + eps);
		a_A_3_avg(8) = 0.0;
	...
	void Spherical_map_filling_func(mapping_variables &m_map_vars, 
								  const Box &bxmap,
								  const double a_dx,
								  const double a_dy,
								  const double a_dz)
	{
		...
		bxmap,
		(m_map_vars.m_Jacobian_ave), 
		(m_map_vars.m_detAA_avg), 
		(m_map_vars.m_detAA_inv_avg), 
		(m_map_vars.m_r2rdot_avg), 
		(m_map_vars.m_detA_avg), 
		(m_map_vars.m_A_row_mag_avg), 
		(m_map_vars.m_Dr_detA_avg[0]), 
		(m_map_vars.m_Dr_detA_A_avg[0]), 
		(m_map_vars.m_Dr_AdjA_avg[0]), 
		(m_map_vars.m_A_row_mag_face_avg[0]), 
		(m_map_vars.m_A_1_avg), 
		(m_map_vars.m_Dr_detA_avg[1]), 
		(m_map_vars.m_Dr_detA_A_avg[1]), 
		(m_map_vars.m_Dr_AdjA_avg[1]), 
		(m_map_vars.m_A_row_mag_face_avg[1]), 
		(m_map_vars.m_A_2_avg), 
		(m_map_vars.m_Dr_detA_avg[2]), 
		(m_map_vars.m_Dr_detA_A_avg[2]), 
		(m_map_vars.m_Dr_AdjA_avg[2]), 
		(m_map_vars.m_A_row_mag_face_avg[2]), 
		(m_map_vars.m_A_3_avg), 
		a_dx, 
		a_dy, 
		a_dz);
	...
	void JU_to_U_Sph_ave_calc_func(BoxData<double, NUMCOMPS> &a_U_Sph_ave,
								   ...
								   const BoxData<double, DIM * DIM> &a_detAA_inv_avg,
								   const BoxData<double, 1> &a_r2rdot_avg,
								   const BoxData<double, 1> &a_detA_avg,
								   const BoxData<double, DIM> &a_A_row_mag_avg,
								   bool a_normalized,
								   int a_order)
	{
		if (a_order == 2)
...
#define CENG 4
			...
			a_U_Sph_ave = forall<double, NUMCOMPS, MEM, 1>([] PROTO_LAMBDA(
...
#define CENG 4
			...
			a_JU_ave = forall<double, NUMCOMPS, MEM, 1>([] PROTO_LAMBDA(
															Var<double, NUMCOMPS, MEM, 1> & a_retval,
	...
	void W_Sph_to_W_normalized_sph_calcF(const Point &a_pt,
										 ...
										 const Var<double, DIM> &a_A_row_mag_face_avg,
										 int a_d)
	{
		W_normalized_sph(0) = W_Sph(0);
		W_normalized_sph(1) = W_Sph(1) * a_A_row_mag_face_avg(0);
		W_normalized_sph(2) = W_Sph(2) * a_A_row_mag_face_avg(1);
		W_normalized_sph(3) = W_Sph(3) * a_A_row_mag_face_avg(2);
		W_normalized_sph(4) = W_Sph(4);
		W_normalized_sph(5) = W_Sph(5) * a_A_row_mag_face_avg(0);
		W_normalized_sph(6) = W_Sph(6) * a_A_row_mag_face_avg(1);
		W_normalized_sph(7) = W_Sph(7) * a_A_row_mag_face_avg(2);
	}
	...
	void W_Sph_to_W_normalized_sph(BoxData<double, NUMCOMPS> &W_normalized_sph,
								   ...
								   int a_d)
	{
		forallInPlace_p(W_Sph_to_W_normalized_sph_calc, W_normalized_sph, W_Sph, a_A_row_mag_face_avg, a_d);
	}

	PROTO_KERNEL_START
	void W_normalized_sph_to_W_Sph_calcF(const Point &a_pt,
										 Var<double, NUMCOMPS> &W_sph,
										 Var<double, NUMCOMPS> &W_normalized_Sph,
										 const Var<double, DIM> &a_A_row_mag_face_avg,
										 int a_d)
	{
		double eps = 1e-15;
		W_sph(0) = W_normalized_Sph(0);
		W_sph(1) = W_normalized_Sph(1) / (a_A_row_mag_face_avg(0) + eps);
		W_sph(2) = W_normalized_Sph(2) / (a_A_row_mag_face_avg(1) + eps);
		W_sph(3) = W_normalized_Sph(3) / (a_A_row_mag_face_avg(2) + eps);
		W_sph(4) = W_normalized_Sph(4);
		W_sph(5) = W_normalized_Sph(5) / (a_A_row_mag_face_avg(0) + eps);
		W_sph(6) = W_normalized_Sph(6) / (a_A_row_mag_face_avg(1) + eps);
		W_sph(7) = W_normalized_Sph(7) / (a_A_row_mag_face_avg(2) + eps);
	}
	PROTO_KERNEL_END(W_normalized_sph_to_W_Sph_calcF, W_normalized_sph_to_W_Sph_calc)
	...
	void W_normalized_sph_to_W_Sph(BoxData<double, NUMCOMPS> &W_sph,
								   ...
								   int a_d)
	{
		forallInPlace_p(W_normalized_sph_to_W_Sph_calc, W_sph, W_normalized_Sph, a_A_row_mag_face_avg, a_d);
	}

	void Regular_map_filling_func(mapping_variables &m_map_vars, 
								  const Box &bxmap,
								  const double a_dx,
								  const double a_dy,
								  const double a_dz)
	{
		...
		for (int dir = 0; dir < DIM; dir++)
		{
			m_map_vars.m_NT[dir] = Operator::cofactor(X, dir);
			// h5.writePatch(1,m_map_vars.m_NT[dir],"m_NT"+to_string(dir));
		}
		m_map_vars.m_J = Operator::jacobian(X, m_map_vars.m_NT);
#if DIM == 2
		m_map_vars.m_J *= 1.0 / (a_dx * a_dy);
#endif
#if DIM == 3
		m_map_vars.m_J *= 1.0 / (a_dx * a_dy * a_dz);
#endif
	...
	void Spherical_2O_map_filling_func(mapping_variables &a_state, 
								  const Box &a_bx,
								  const double a_dx,
								  const double a_dy,
								  const double a_dz)
	{
		...
		MHD_Mapping::get_cell_volume(a_state.m_cell_volume, a_bx, a_dx, a_dy, a_dz);
		MHD_Mapping::get_face_area(a_state.m_face_area, a_bx, a_dx, a_dy, a_dz);
		MHD_Mapping::get_delta_sph_coords(a_state.m_dx_sph, a_bx, a_dx, a_dy, a_dz);
		MHD_Mapping::get_sph_coords_cc(a_state.m_x_sph_cc, a_bx, a_dx, a_dy, a_dz);
		MHD_Mapping::get_sph_coords_fc(a_state.m_x_sph_fc_1, a_bx, a_dx, a_dy, a_dz, 0);
		MHD_Mapping::get_sph_coords_fc(a_state.m_x_sph_fc_2, a_bx, a_dx, a_dy, a_dz, 1);
		MHD_Mapping::get_sph_coords_fc(a_state.m_x_sph_fc_3, a_bx, a_dx, a_dy, a_dz, 2);
